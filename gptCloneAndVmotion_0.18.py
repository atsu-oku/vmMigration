            ...
            for fpath, content_var in [(stdout_path, "stdout_content"), (stderr_path, "stderr_content")]:
                try:
                    ...
                    if resp.status_code == 200:
                        if content_var == "stdout_content": stdout_content = resp.text
                        else: stderr_content = resp.text
                except (vim.fault.FileNotFound, requests.exceptions.RequestException):
                    pass
                finally:
                    try: file_manager.DeleteFileInGuest(vm=vm, auth=auth, filePath=fpath)
                    except (vim.fault.FileNotFound, vim.fault.GuestOperationsFault): pass
        
        return exit_code, stdout_content.strip(), stderr_content.strip()

    # ---- Main logic of the function ----
# Present command before issuing
print("[GUEST-CMD] 発行予定コマンド:")
print(f"  {command}")
...
try:
    ...
    if exit_code != 0 and check_exit_code:
        # After issuing: show OS return and reason
        print(f"[GUEST-CMD] 実行ユーザー: {auth_used}")
        print(f"[GUEST-CMD] 終了コード: {exit_code}")
        print("[GUEST-CMD] 標準出力:\n---\n" + (stdout or "(なし)") + "\n---")
        print("[GUEST-CMD] 標準エラー:\n---\n" + (stderr or "(なし)") + "\n---")
        print(f"[GUEST-CMD] 結果: 異常終了 (理由: adminでの実行失敗)")
        raise RuntimeError(f"adminとしてのコマンド実行に失敗 (Exit Code: {exit_code})") from e
...
if exit_code == 0:
    print("[GUEST-CMD] 結果: 正常終了")
else:
    print("[GUEST-CMD] 結果: 異常終了")
    if check_exit_code:
