...
def _is_link_local_source(source: str) -> bool:
    ...
    try:
        if "/" in candidate:
            ...
            return isinstance(network, ipaddress.IPv4Network) and network.subnet_of(LINK_LOCAL_PREFIX)
        address = ipaddress.ip_address(candidate)
        return isinstance(address, ipaddress.IPv4Address) and address in LINK_LOCAL_PREFIX
    except ValueError:
        return False
