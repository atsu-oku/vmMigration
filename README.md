...
- Keep additional guest files immutable with `--protect-guest-file /path`. Use `--enable-standard-config-edits` only when the PRD run must touch the default set (/etc/profile, /etc/hosts, and standard chrony/ntp configs).
