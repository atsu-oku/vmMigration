...
- エラー時はロールバック（クローン削除・不要ファイル削除）を案内

**対応/前提**
- vCenter に API 到達できること
- 対象 VM に VMware Tools が導入・稼働し、Guest Operations が利用可能
- ゲスト OS 側に `nmcli`（NetworkManager）が利用可能であること（主に Linux）
- Python 3.11 以上

**依存モジュール**
- pyvmomi
- requests
- 導入方法は `requirements.txt` を参照（セットアップ詳細は `SETUP.md`）
...
## 2. 主な設定値（スクリプト内）
以下は `gptCloneAndVmotion.py` 冒頭の定数で指定します。値は環境に合わせて編集してください。
- vCenter 接続
...
- Q. 途中で失敗した場合は？
  - A. 可能な範囲でクローン VM の削除や不要ファイル削除などのロールバック手順を案内します。
...
- ゲスト OS 認証に失敗する
  - VMware Tools で該当アカウントに Guest Operations へのアクセス権があるか確認
  - sudo 設定（パスワード要求/免除）を確認
- 詳細ログを見たい
