...
vCenter 上の VM を STG 環境から PRD 環境へ移行するためのスクリプト群です。

## 更新履歴
- 2025-10-10: pyVim と pyvim の両方に対応できるようフォールバックを追加。
...
## ネットワーク名マッピング例（STG → PRD）
- 変換ロジック: スクリプトは「先頭の 'STG' を 'PRD' に置換」して宛先のネットワーク名を探索します（eplace('STG','PRD', 1)）。
- 例（標準/分散PG共通の命名を想定）
  - STG-SEG-APP → PRD-SEG-APP
  - STG-SEG-WEB → PRD-SEG-WEB
  - STG-DB-01 → PRD-DB-01
  - STG-DMZ-PROD → PRD-DMZ-PROD
- 注意事項
  - 大文字小文字は一致が前提（STG/PRD）です。別の命名（Stg など）が混在する場合は事前に正規化してください。
  - 宛先に同名のPRDネットワークが存在しない場合はエラーになります。必要に応じてPGの作成やマッピング表の事前整備をご検討ください。
  - 分散ポートグループ（DVS）の場合は、PGキーとスイッチUUIDを用いて接続します（スクリプトで自動判別）。
