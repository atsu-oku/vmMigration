...
def register_command_execution(command: str) -> None:
    ...
    COMMAND_EXECUTION_LOG.append(
        CommandLogEntry(command=command, description=description)
    )
...
def _parse_cli_arguments(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Allow the workflow to modify the default set of protected system files "
            "(/etc/profile, /etc/hosts, and standard NTP configurations)."
        ),
    )
    parser.add_argument(
        "--protect-guest-file",
        dest="protected_guest_files",
        action="append",
        default=[],
        metavar="PATH",
        help="Additional absolute guest file path to skip (can be supplied multiple times).",
    )
...
def _print_execution_summary() -> None:
    ...
    if COMMAND_EXECUTION_LOG:
        ...
        for index, entry in enumerate(COMMAND_EXECUTION_LOG, 1):
            command_single_line = " ".join(
                line.strip() for line in entry.command.strip().splitlines()
            )
...
def _load_local_module(module_name: str, filename: str):
    ...
    if spec is None or spec.loader is None:
        raise ImportError(
            f"Unable to load specification for {module_name} at {module_path}"
        )
...
try:
    ...
    try:
        vsphere_sdk_network = _load_local_module(
            "vsphere_sdk_network", "vsphere_sdk_network.py"
        )
    except Exception as load_error:  # pylint: disable=broad-exception-caught
        raise import_error from load_error
...
try:
    ...
    verify_destination_network_with_sdk = (
        network_utils.verify_destination_network_with_sdk
    )
...
DEFAULT_PROTECTED_GUEST_PATHS: Tuple[str, ...] = (
    "/etc/profile",
    "/etc/hosts",
    "/etc/chrony.conf",
...
_PROTECTED_GUEST_PATH_LOOKUP: Set[str] = set()


def _normalize_guest_path(path: str) -> str:
    """Return a normalized POSIX path suitable for protected path comparisons."""
...
def register_protected_guest_paths(paths: Iterable[str]) -> None:
    """Collect additional guest paths that must not be mutated."""
    for candidate in paths:
        normalized = _normalize_guest_path(candidate)
        if normalized:
            _PROTECTED_GUEST_PATH_LOOKUP.add(normalized)


def unregister_protected_guest_paths(paths: Iterable[str]) -> None:
    """Remove guest paths from the protected list when explicit overrides are requested."""
    for candidate in paths:
        normalized = _normalize_guest_path(candidate)
        if normalized:
            _PROTECTED_GUEST_PATH_LOOKUP.discard(normalized)


register_protected_guest_paths(DEFAULT_PROTECTED_GUEST_PATHS)


def _guest_path_protected(path: str) -> bool:
    """Return True when the target guest path must not be modified."""
...
def _interface_has_odd_third_octet(
    guest_executor,
    interface_name: str,
    cache: Dict[str, bool],
) -> bool:
    """Return True when the interface owns an IPv4 whose third octet is odd."""
    if interface_name in cache:
        return cache[interface_name]
    command = f"ip -4 addr show dev {shlex.quote(interface_name)}"
    exit_code, stdout, stderr = guest_executor(command, check_exit_code=False)
    result = False
    if exit_code == 0:
        ...
        if stderr:
            LOGGER.debug(
                "Unable to inspect IPv4 addresses for interface '%s': %s",
                interface_name,
                stderr.strip(),
            )
    cache[interface_name] = result
    return result
...
NTP_SERVER_DIRECTIVE_PATTERN = re.compile(
    r"^\s*(?:server|pool)\s+(\S+)", re.IGNORECASE | re.MULTILINE
)
...
TD_AGENT_BASEURL_TEMPLATE = (
    "https://packages.treasuredata.com/{major}/redhat/$releasever/$basearch"
)
    ...
    *,
    ...
    try:
        session = SmartConnect(
            host=host, user=user, pwd=password, port=VCSA_PORT, sslContext=ssl_ctx
        )
        ...
        raise ConnectionError(
            f"Unable to resolve vCenter host '{host}'. {hint}"
        ) from error
    if not session:
...
VCENTER_KEEPALIVE_SECONDS = int(
    os.environ.get("VSPHERE_CLONE_KEEPALIVE_SECONDS", "240")
)
...
def _start_keepalive_thread(
    ...
    thread = threading.Thread(
        target=_keepalive_loop, name=f"{label}-keepalive", daemon=True
    )
    thread.start()
...
def _stop_keepalive_thread(
    ...
    if thread.is_alive():
        LOGGER.warning(
            "Keep-alive thread '%s' failed to terminate cleanly.", thread.name
        )
        return False
    return True
...
def wait_for_task_completion(
    ...
    *,
    ...
    while True:
        ...
        if now >= deadline:
            raise TimeoutError(
                f"{description} did not complete within {int(timeout)} seconds."
            )
        if label:
            current_progress = getattr(info, "progress", None)
            if (
                isinstance(current_progress, int)
    ...
    if raise_on_error and state != vim.TaskInfo.State.success:
        ...
        error_message = (
            getattr(error, "msg", None) or str(error) if error else "unknown error"
        )
...
def _maybe_rename_guest_interface(
    ...
    print(
        f"   -> Renaming guest interface '{device_name}' to '{desired_ifname_clean}' to match source."
    )
    rename_failed = False
    ...
    print(
        f"   -> Interface rename completed; proceeding with '{desired_ifname_clean}'."
    )
...
def prepare_guest_interface(  # pylint: disable=redefined-outer-name,too-many-locals
    ...
    if not device_name:
        ...
        raise RuntimeError(
            f"Unable to locate guest interface matching MAC {target_mac}"
        )
...
def _is_service_active(guest_executor, service_name: str) -> bool:
    ...
    if exit_code == 0:
        exit_code, stdout, _ = guest_executor(
            f"systemctl is-active {service_quoted}", check_exit_code=False
        )
        if exit_code == 0 and (stdout or "").strip() == "active":
            return True
    exit_code, _, _ = guest_executor(
        f"service {service_quoted} status >/dev/null 2>&1", check_exit_code=False
    )
    return exit_code == 0
...
def _ensure_guest_write_privileges(guest_executor) -> bool:
    ...
    if exit_code == 0 and (stdout or "").strip() == "0":
        return True
    probe_exit, _, probe_err = guest_executor(
        "test -w /etc && printf ok", check_exit_code=False
    )
    if probe_exit == 0:
        return True
    detail = (
        (stderr or "").strip()
        or (probe_err or "").strip()
        or f"id -u exit code {exit_code}"
    )
    print(
        f"   [ERROR] Guest executor lacks root access to modify system configuration: {detail}"
    )
    log_failure(
        "Guest credentials do not provide root privileges; system configuration sync skipped."
    )
    return False


def _write_guest_file(
    guest_executor, remote_path: str, content: str
) -> Tuple[int, str, str]:
    """Write content to a file inside the guest using sequential bash commands."""
    if _guest_path_protected(remote_path):
        protected_path = _normalize_guest_path(remote_path)
        print(f"   -> Skipping update for protected path {protected_path}.")
        log_success(f"Skipped guest file update for protected path {protected_path}.")
        return 0, "", ""
    ...
    if tmp_exit != 0 or not (tmp_stdout or "").strip():
        tmp_exit, tmp_stdout, tmp_err = guest_executor(
            "mktemp /tmp/.vsphere_tmp.XXXXXX", check_exit_code=False
        )
        if tmp_exit != 0 or not (tmp_stdout or "").strip():
            return tmp_exit or 1, "", tmp_err
    tmp_path = (tmp_stdout or "").strip().splitlines()[-1]
    remember_command_description(
        f"mktemp {tmp_path}", f"{tmp_path} を一時ファイルとして確保"
    )
    ...
    remember_command_description(
        stat_cmd, f"{remote_path} のパーミッションと所有者を取得"
    )
    ...
    if owner_spec is None:
        ...
        owner_spec = (
            (uid_stdout or "").strip()
            if uid_exit == 0 and (uid_stdout or "").strip()
            else "0"
        )
    ...
    if group_spec is None:
        ...
        group_spec = (
            (gid_stdout or "").strip()
            if gid_exit == 0 and (gid_stdout or "").strip()
            else "0"
        )

    chmod_cmd = f"chmod {shlex.quote(perm_spec)} {shlex.quote(tmp_path)}"
    remember_command_description(
        chmod_cmd, f"{tmp_path} のパーミッションを {perm_spec} に設定"
    )
    ...
    remember_command_description(
        chown_cmd, f"{tmp_path} の所有者を {owner_spec}:{group_spec} に設定"
    )
...
def _rewrite_centos_repo_content(repo_content: str) -> Tuple[str, bool]:
    ...
    for line in lines:
        ...
        if mirror_match:
            ...
            disabled_line = (
                f"{indent}# mirrorlist={definition} (disabled: vault.centos.org)"
            )
...
def _run_curl_with_tls_repairs(
    ...
    *,
    ...
    for _ in range(max_attempts):
        curl_cmd = f"curl -Is --max-time 10 --retry 2 --retry-delay 2 {args} {shlex.quote(url)} >/dev/null"
...
def _sync_hosts_file_to_prd(guest_executor, timestamp: str) -> bool:
    ...
    if _guest_path_protected("/etc/hosts"):
        print(
            "   -> Skipping /etc/hosts synchronisation because the path is protected."
        )
        return True
    exit_code, hosts_content, hosts_err = guest_executor(
        "cat /etc/hosts", check_exit_code=False
    )
    if exit_code != 0:
        print(
            f"   [WARN] Unable to read /etc/hosts: {(hosts_err or '').strip() or exit_code}"
        )
        return False
    ...
    if not changed:
        ...
        if " line-" in hosts_content and re.search(
            r"\bline-[^ \t\n]*s\b", hosts_content
        ):
        ...
        if indicators:
            print(
                f"   [WARN] Detected residual STG markers in /etc/hosts ({', '.join(indicators)})."
            )
        else:
    ...
    backup_cmd = (
        f"[ -f {shlex.quote(backup_path)} ] || cp /etc/hosts {shlex.quote(backup_path)}"
    )
    backup_exit, _, backup_err = guest_executor(backup_cmd, check_exit_code=False)
    if backup_exit != 0:
        print(
            f"   [WARN] Unable to create /etc/hosts backup: {(backup_err or '').strip() or backup_exit}"
        )
        return False
    write_exit, _, write_err = _write_guest_file(
        guest_executor, "/etc/hosts", updated_hosts
    )
    if write_exit != 0:
        print(
            f"   [WARN] Failed to update /etc/hosts: {(write_err or '').strip() or write_exit}"
        )
        return False
...
def _sync_firewalld_configuration_to_prd(
    ...
    if exit_code != 0:
        print(
            "   -> firewall-cmd not available; skipping firewalld configuration sync."
        )
        return True
    ...
    if zones_updated:
        reload_exit, _, reload_err = guest_executor(
            "firewall-cmd --reload", check_exit_code=False
        )
        if reload_exit != 0:
            detail = (reload_err or "").strip() or reload_exit
            print(
                f"   [WARN] Firewalld reload failed after SSH override updates: {detail}"
            )
            overall_success = False
        else:
            print(
                f"   -> Firewalld reloaded after updating zones: {', '.join(sorted(zones_updated))}"
            )
    else:
        print(f"   -> No zone required SSH overrides for {SSH_OVERRIDE_SOURCE_IP}.")
...
def _sync_ntp_configuration_to_prd(guest_executor, timestamp: str) -> bool:
    ...
    for config_path in NTP_CONFIG_PATHS:
        if _guest_path_protected(config_path):
            print(
                f"   -> Skipping NTP configuration sync for protected path {config_path}."
            )
            found_config = True
            continue
        exists_exit, _, _ = guest_executor(
        ...
        if exit_code != 0:
            print(
                f"   [WARN] Unable to read {config_path}: {(config_err or '').strip() or exit_code}"
            )
            success = False
            break
        found_config = True
        ...
        if stage_servers_after and expected_conversions:
            for stage_ip in stage_servers_after:
                replacement = expected_conversions.get(
                    stage_ip
                ) or calculate_ip_stg_to_prd(stage_ip)
        ...
        if not changed:
            if (
                not stage_servers_after
                and not conversion_failures
                and not conversion_anomalies
            ):
        ...
        if backup_exit != 0:
            print(
                f"   [WARN] Unable to back up {config_path}: {(backup_err or '').strip() or backup_exit}"
            )
            success = False
            break
        write_exit, _, write_err = _write_guest_file(
            guest_executor, config_path, updated_config
        )
        if write_exit != 0:
            print(
                f"   [WARN] Failed to update {config_path}: {(write_err or '').strip() or write_exit}"
            )
            success = False
            break
        print(
            f"   -> Updated {config_path} NTP servers for PRD environment (backup: {backup_path})."
        )
        break
    if not found_config:
...
def _sync_centos_repo_configuration(guest_executor, timestamp: str) -> bool:
    ...
    if _guest_path_protected(CENTOS_REPO_DIR):
        print("   -> Skipping CentOS repo sync; handled by external automation.")
        return True
    repo_dir_check, _, _ = guest_executor(
        f"test -d {shlex.quote(CENTOS_REPO_DIR)}", check_exit_code=False
    )
    if repo_dir_check != 0:
    ...
    if not curl_ok:
        print(
            f"   [WARN] Unable to reach vault.centos.org even after TLS repairs: {curl_error or 'no details'}"
        )
    ...
    for repo_path in ls_stdout.splitlines():
        ...
        if not repo_path:
            continue
        exit_code, repo_content, repo_err = guest_executor(
            f"cat {shlex.quote(repo_path)}", check_exit_code=False
        )
        if exit_code != 0:
            print(
                f"   [WARN] Unable to read {repo_path}: {(repo_err or '').strip() or exit_code}"
            )
            success = False
            continue
        ...
        if backup_exit != 0:
            print(
                f"   [WARN] Unable to back up {repo_path}: {(backup_err or '').strip() or backup_exit}"
            )
            success = False
            continue
        write_exit, _, write_err = _write_guest_file(
            guest_executor, repo_path, updated_content
        )
        if write_exit != 0:
            print(
                f"   [WARN] Failed to update {repo_path}: {(write_err or '').strip() or write_exit}"
            )
            success = False
            continue
        print(
            f"   -> Updated CentOS repo file '{repo_path}' to use vault.centos.org (backup: {backup_path})."
        )
    return success
...
def _ensure_td_agent_repo(guest_executor, timestamp: str) -> bool:
    ...
    if _guest_path_protected(TD_AGENT_REPO_PATH):
        print(
            f"   -> Skipping td-agent repo configuration; {TD_AGENT_REPO_PATH} managed externally."
        )
        return True
    ...
    if not _test_major(selected_major):
        ...
        if not _test_major(selected_major):
            print(
                "   [WARN] td-agent v3 repository check failed as well; proceeding with v3 definition."
            )
    ...
    write_exit, _, write_err = _write_guest_file(
        guest_executor, TD_AGENT_REPO_PATH, repo_content
    )
    if write_exit != 0:
        print(
            f"   [WARN] Failed to write td-agent repo file: {(write_err or '').strip() or write_exit}"
        )
        return False
    print(
        f"   -> td-agent repository configured for major v{selected_major} (path: {TD_AGENT_REPO_PATH})."
    )
    return True
...
def _sync_iptables_configuration_to_prd(guest_executor, timestamp: str) -> bool:
    ...
    if exit_code != 0:
        print(
            "   -> iptables command unavailable; skipping iptables configuration sync."
        )
        return True
    health_code, iptables_state, health_err = guest_executor(
        "iptables -S", check_exit_code=False
    )
    if health_code != 0:
        print(
            f"   [WARN] Unable to inspect iptables rules: {(health_err or '').strip() or health_code}"
        )
        return False
    lines = [
        line.strip() for line in (iptables_state or "").splitlines() if line.strip()
    ]
    ssh_accept_lines = [
        line
        for line in lines
    ...
    if conflicting_accepts:
        ...
        return False
    matching_rule = any(
        line for line in ssh_accept_lines if f"-s {SSH_OVERRIDE_SOURCE_IP}" in line
    )
    if matching_rule:
        print("   -> iptables already restricts SSH to the approved source.")
        return True
    add_rule_cmd = (
        f"iptables -I INPUT 1 -p tcp -s {SSH_OVERRIDE_SOURCE_IP} --dport 22 -j ACCEPT"
    )
    add_exit, _, add_err = guest_executor(add_rule_cmd, check_exit_code=False)
    if add_exit != 0:
        print(
            f"   [WARN] Failed to add iptables SSH allow rule: {(add_err or '').strip() or add_exit}"
        )
        return False
...
def _sync_prd_system_configuration(
    ...
    print(
        "   -> Synchronising guest hosts and firewall configuration with PRD mappings..."
    )
    ...
    if not _sync_hosts_file_to_prd(guest_executor, timestamp):
        success = False
    if not _sync_firewalld_configuration_to_prd(
        guest_executor, timestamp, source_zones
    ):
        success = False
...
def _ensure_http_proxy_configuration(guest_executor, timestamp: str) -> bool:
    ...
    if _guest_path_protected(profile_path):
        print(
            f"   -> Skipping proxy configuration updates because {profile_path} is protected."
        )
        return True
    ...
    if profile_exists:
        ...
        if profile_exit != 0:
            print(
                f"   [WARN] Unable to read {profile_path}: {(profile_err or '').strip() or profile_exit}"
            )
            return False
    else:
        profile_content = ""
    ...
    env_exit, env_stdout, _ = guest_executor(
        "env | grep -i http", check_exit_code=False
    )
    ...
    if proxies_rewritten:
        ...
        if backup_exit != 0:
            print(
                f"   [WARN] Unable to back up {profile_path}: {(backup_err or '').strip() or backup_exit}"
            )
            return False
        if not transformed_profile.endswith("\n"):
            transformed_profile += "\n"
        write_exit, _, write_err = _write_guest_file(
            guest_executor, profile_path, transformed_profile
        )
        if write_exit != 0:
            print(
                f"   [WARN] Failed to update {profile_path}: {(write_err or '').strip() or write_exit}"
            )
            return False
        print(
            f"   -> Updated proxy configuration in {profile_path} (backup: {backup_path})."
        )
        reload_exit, _, reload_err = guest_executor(
            f". {profile_path}", check_exit_code=False
        )
        if reload_exit != 0:
            print(
                f"   [WARN] Reloading {profile_path} returned exit code {reload_exit}: {(reload_err or '').strip()}"
            )
        return True
    if has_proxy_env:
        print(
            "   -> Proxy environment variables detected but /etc/profile lacks persistent exports; adding block."
        )
    backup_cmd = (
        f"[ -f {shlex.quote(backup_path)} ] || "
        f"cp {shlex.quote(profile_path)} {shlex.quote(backup_path)}"
    ...
    write_exit, _, write_err = _write_guest_file(
        guest_executor, profile_path, updated_profile
    )
    if write_exit != 0:
        print(
            f"   [WARN] Failed to update {profile_path}: {(write_err or '').strip() or write_exit}"
        ...
        return False
    verify_exit, verify_content, _ = guest_executor(
        f"cat {shlex.quote(profile_path)}", check_exit_code=False
    )
    if verify_exit != 0 or not all(
        line in (verify_content or "") for line in proxy_lines
    ):
        print(
            f"   [WARN] Verification failed after updating {profile_path}; proxy exports may be missing."
        )
        return False
    guest_executor(f". {profile_path}", check_exit_code=False)
    print(f"   -> Added proxy exports to {profile_path} (backup: {backup_path}).")
    env_verify_exit, env_verify_stdout, _ = guest_executor(
        "env | grep -i http", check_exit_code=False
    )
    if env_verify_exit != 0 or proxy_url not in (env_verify_stdout or ""):
        print(
            "   [WARN] Proxy environment variables may not be active in the current session; please re-login."
        )
    return True
...
def collect_source_vm_metadata(
    *,
    ...
    if REQUESTS_AVAILABLE:
        ...
        if sdk_vm_id:
            try:
                ...
                LOGGER.warning(
                    "Failed to collect source VM network info via API: %s", sdk_error
                )
                interfaces = []
    ...
    for device in getattr(target_vm.config.hardware, "device", []):
        ...
        if (
            guest_iface
            and getattr(guest_iface, "ipConfig", None)
            and getattr(guest_iface.ipConfig, "ipAddress", None)
        ):
            ip_v4_info = next(
                (
                    ip
                    for ip in guest_iface.ipConfig.ipAddress
                    if "." in getattr(ip, "ipAddress", "")
                ),
                None,
            )
            if (
                ip_v4_info
                and getattr(ip_v4_info, "ipAddress", None)
                and not nic_ip_address
            ):
                nic_ip_address = ip_v4_info.ipAddress

        subnet_mask = (
            prefix_to_subnet_mask(prefix_len) if prefix_len is not None else None
        )
        if (
            subnet_mask is None
            and ip_v4_info
    ...
    if nic_plans and guest_operations_manager:
        try:
            ...
            def source_guest_command_executor(
                command: str, check_exit_code: bool = True
            ):
    ...
    if source_routes:
        ...
        selected_default = select_default_gateway_route(
            default_route_candidates, nic_plans
        )
        if selected_default:
            ...
            print(
                f"   -> Detected default gateway {original_default_gateway} (NIC {owner_display})."
            )
            ...
            if owner_candidate is None:
                ...
                if owner_candidate is None and original_default_gateway:
                    owner_candidate = find_gateway_owner_index(
                        nic_plans, original_default_gateway
                    )
    ...
    if ip_stack_objects:
        for stack in ip_stack_objects:
            ...
            for route in getattr(route_config, "ipRoute", []):
                ...
                gateway_ip = (
                    getattr(gateway_obj, "ipAddress", None) if gateway_obj else None
                )
                ...
                if key in route_keys:
                    continue
                if _is_link_local_network(
                    network_str, prefix_int
                ) or _is_link_local_address(gateway_ip):
                    continue
    ...
    if default_gateway_owner_idx is not None and 0 <= default_gateway_owner_idx < len(
        nic_plans
    ):
        for idx, nic_plan in enumerate(nic_plans):
...
class CloneAndVmotionWorkflow:
    ...
    def _collect_source_vm_details(self) -> None:
        ...
        if not target_datastore:
            raise FileNotFoundError(
                f"Datastore '{TARGET_DATASTORE_NAME}' was not found."
            )
    ...
    def _confirm_clone_plan(self) -> None:
        ...
        print(
            "Review the details below before creating the clone and starting the migration."
        )
        print("\n  [Source VM details]")
        ...
        if self.state.original_nic_info:
            for nic_index, nic_details in enumerate(self.state.original_nic_info):
                ...
                print(
                    f"      - IP Address  : {nic_details.get('ip_address', '(unknown)')}"
                )
                print(
                    f"      - Subnet Mask : {nic_details.get('subnet_mask', '(unknown)')}"
                )
        else:
        ...
        if self.state.original_default_gateway:
            ...
            if gateway_source in ("octet-fallback", "nic-gateway"):
                print(
                    f"    - Gateway     : (not reported; fallback -> {gateway_display})"
                )
    ...
    def _perform_source_clone_operations(self) -> None:
        ...
        target_folder = self.state.target_folder or getattr(
            self.target_vm, "parent", None
        )

        source_relocate_spec = vim.vm.RelocateSpec(datastore=selected_datastore)
        source_clone_spec = vim.vm.CloneSpec(
            location=source_relocate_spec, powerOn=False, template=False
        )

        print("\nStarting clone task...")
        clone_task = self.target_vm.Clone(
            folder=target_folder, name=planned_clone_name, spec=source_clone_spec
        )
        ...
        nic_devices = [
            dev
            for dev in cloned_vm.config.hardware.device
    ...
    def _register_destination_vm(self) -> None:
        ...
        if not state.clone_name or not state.vmx_path:
            raise RuntimeError(
                "Clone metadata is incomplete; source operations may have failed."
            )
        ...
        state.dest_keepalive_handle = _start_keepalive_thread(
            self.si_dest, "dest-vcenter"
        )
        ...
        if not dest_cluster:
            raise FileNotFoundError(
                f"Destination cluster '{TARGET_CLUSTER_NAME}' was not found."
            )
        ...
        state.migrated_vm = wait_for_vm_availability(
            self.content_dest, state.clone_name, retries=60, delay_seconds=2
        )
        ...
        *,
        ...
        if session is None:
            ...
            try:
                ...
                LOGGER.debug(
                    "Destination vCenter health check failed; will attempt reconnect.",
                    exc_info=True,
                )
        ...
        if self.si_dest is not None:
            try:
                ...
                LOGGER.debug(
                    "Ignoring error while disconnecting stale destination session.",
                    exc_info=True,
                )
    ...
    def _recreate_destination_nics(self) -> None:
        ...
        if not migrated_vm or not self.content_dest:
            raise RuntimeError(
                "Destination VM context is not available; registration may have failed."
            )

        print("\n--- [Phase 4/7] Destination vCenter: Reconfigure NICs ---")
        if not state.original_nic_info:
            print(
                "   - Skipping NIC reconfiguration because the original VM had no NICs."
            )
            return
        ...
        for index, nic_plan in enumerate(state.original_nic_info):
            ...
            print(
                f"  - NIC {index + 1}: '{original_network_name}' -> '{dest_network_name}'"
            )
            ...
            if isinstance(dest_network, vim.dvs.DistributedVirtualPortgroup):
                ...
                raise FileNotFoundError(
                    f"Destination network '{dest_network_name}' was not found."
                )
        ...
        if not device_change_spec:
            print(
                "   - Skipping NIC reconfiguration because no NIC specifications were prepared."
            )
            return
        ...
        try:
            ...
            migrated_vm = wait_for_vm_availability(
                self.content_dest, state.clone_name, retries=30, delay_seconds=2
            )
        ...
        for nic_plan, (dest_name, dest_identifier) in zip(
            state.original_nic_info, dest_network_lookup
        ):
            matched_nic = None
            for nic_device in remaining_new_nics:
                ...
                if isinstance(
                    ...
                    actual_name = getattr(backing, "deviceName", None) or getattr(
                        network_obj, "name", None
                    )
                else:
            ...
            if matched_nic is None:
                if not remaining_new_nics:
                    raise RuntimeError(
                        "Unable to associate newly created NICs with destination networks."
                    )
...
def main() -> None:
    ...
    if getattr(cli_args, "enable_standard_config_edits", False):
        unregister_protected_guest_paths(DEFAULT_PROTECTED_GUEST_PATHS)
        print(
            "[INFO] Standard config protection disabled; sensitive guest files may be modified."
        )
    additional_protected = getattr(cli_args, "protected_guest_files", None) or []
    if additional_protected:
        register_protected_guest_paths(additional_protected)
        for protected_path in additional_protected:
            print(
                f"[INFO] Protecting guest file: {_normalize_guest_path(protected_path)}"
            )
    ...
    # ------------------------------------------------
    try:
        VCSA_PWD_SOURCE = getpass.getpass(
            f"Password for {VCSA_USER} on {VCSA_HOST_SOURCE}: "
        )
        VCSA_PWD_DEST = getpass.getpass(
            f"Password for {VCSA_USER} on {VCSA_HOST_DEST}: "
        )
    ...
    try:
        GUEST_ROOT_PWD = getpass.getpass(
            f"Password for Guest OS user '{GUEST_ROOT_USER}': "
        )
    ...
    try:
        ...
        if original_nic_info:
            for i, nic in enumerate(original_nic_info):
                is_gateway_nic = (
                    ...
                    and i == default_gateway_owner_idx
                )
                ...
                if is_gateway_nic:
                    print("      - Default GW  : owns default gateway route")
        else:
            print("    - No NIC information was found.")
        if source_dns_servers:
            print("\n  [Source DNS servers]")
            for dns_server in source_dns_servers:
                print(f"    - {dns_server}")
        else:
            print("\n  [Source DNS servers]")
            print("    - None reported.")
        if original_default_gateway:
            if original_default_gateway_source in ("octet-fallback", "nic-gateway"):
                print(
                    f"    - Gateway     : (not reported; fallback -> {original_default_gateway})"
                )
            elif original_default_gateway_source == "route-inferred":
        ...
        print("\nStarting clone task...")
        task = target_vm.Clone(
            folder=target_vm.parent, name=clone_name, spec=clone_spec
        )
        wait_for_task_completion(
            task,
            "Clone task",
        ...
        nic_devices_to_remove = [
            dev
            for dev in new_vm_on_source.config.hardware.device
            if isinstance(dev, vim.vm.device.VirtualEthernetCard)
        ]
        if nic_devices_to_remove:
            nic_change_spec = [
                vim.vm.device.VirtualDeviceSpec(operation="remove", device=nic)
                for nic in nic_devices_to_remove
            ]
        ...
        migrated_vm = wait_for_vm_availability(
            content_dest, clone_name, retries=60, delay_seconds=2
        )
        ...
        if original_nic_info:
            ...
            for i, nic in enumerate(original_nic_info):
                ...
                dest_network_name = original_network_name.replace("STG", "PRD", 1)
                print(
                    f"  - NIC {i + 1}: '{original_network_name}' -> '{dest_network_name}'"
                )
                ...
                if user_approval_nic.lower() != "y":
                    raise InterruptedError(
                        "NIC configuration was cancelled by the user."
                    )
                if isinstance(dest_network, vim.dvs.DistributedVirtualPortgroup):
                    dest_identifier = (
                        "dvs",
            ...
            if not device_change_spec:
                ...
                try:
                    ...
                    migrated_vm = wait_for_vm_availability(
                        content_dest, clone_name, retries=30, delay_seconds=2
                    )
            ...
            remaining_new_nics = list(newly_added_nics)
            for nic_record, (dest_name, dest_identifier) in zip(
                original_nic_info, dest_network_lookup
            ):
                matched_nic = None
                for nic_dev in remaining_new_nics:
                    ...
                    if isinstance(
                        ...
                        actual_identifier = (
                            "network",
                            getattr(network_obj, "_moId", None),
                        )
                        actual_name = getattr(backing, "deviceName", None) or getattr(
                            network_obj, "name", None
                        )
        ...
        gateway_nic_present = any(
            nic.get("is_gateway_nic") for nic in original_nic_info
        )
        ...
        if prd_static_routes:
            ...
            for route_info in prd_static_routes:
                owner_index = route_info.get("owner_index")
                owner_label = (
                    f"NIC #{owner_index + 1}" if owner_index is not None else "Any NIC"
                )
                ...
                print(
                    f"      - {destination} via {route_info['gateway']} ({owner_label})"
                )
        ...
        if new_default_gateway:
            ...
            print(
                f"    - Gateway     : {original_default_gateway} -> {new_default_gateway}"
            )
        ...
        if user_approval_ip.lower() != "y":
            raise InterruptedError(
                "IP configuration and power-on were cancelled by the user."
            )
        ...
        for i in range(10):
            ...
            try:
                creds_check = vim.vm.guest.NamePasswordAuthentication(
                    username=GUEST_ROOT_USER, password=GUEST_ROOT_PWD
                )
                process_manager = guest_operations_manager.processManager
                spec_check = vim.vm.guest.ProcessManager.ProgramSpec(
                    programPath="/bin/echo", arguments="ready"
                )
                pid = process_manager.StartProgramInGuest(
                    vm=migrated_vm, auth=creds_check, spec=spec_check
                )
                if pid >= 0:
                    agent_ready = True
                    break
        ...
        if original_nic_info:
            ...
            def guest_command_executor(command, check_exit_code=True):
                ...
                try:
                    ...
                    evaluated_vm = wait_for_vm_availability(
                        content_dest, clone_name, retries=30, delay_seconds=2
                    )
            ...
            expected_dns_overall: List[str] = []
            nmcli_validation_tasks: List[
                Tuple[str, str, str, Optional[str], List[str], List[str]]
            ] = []
            if REQUESTS_AVAILABLE:
                try:
                    ...
                    if sdk_vm_id:
                        ...
                        if sdk_interfaces:
                            ...
                            print(
                                "   -> Reconfiguring guest networking via the vSphere Automation SDK."
                            )
                        else:
                            print(
            ...
            for i, nic_info in enumerate(original_nic_info):
                new_ip = nic_info.get("prd_ip_address") or calculate_ip_stg_to_prd(
                    nic_info["ip_address"]
                )
                ...
                if subnet_mask:
                    try:
                        prefix = sum(
                            bin(int(part)).count("1") for part in subnet_mask.split(".")
                        )
                    except ValueError:
                ...
                interface_ctx = prepare_guest_interface(
                    i, nic_info, guest_command_executor, new_mac
                )
                ...
                nmcli_check_exit, _, _ = guest_command_executor(
                    "command -v nmcli", check_exit_code=False
                )
                nmcli_supported = nmcli_check_exit == 0
                new_dns_servers: List[str] = []
                if i == 0 and original_dns_servers:
                    new_dns_servers = dedupe_preserving_order(
                        calculate_ip_stg_to_prd(dns)
                        for dns in original_dns_servers
                        if dns
                    )
                    if new_dns_servers:
                ...
                if not use_nmcli_connection and not legacy_config_success:
                    raise RuntimeError(
                        "Legacy network configuration failed inside the guest OS. See log output above."
                    )
                ...
                if use_nmcli_connection:
                    ...
                    if up_exit != 0:
                        combined_output = "\n".join(
                            filter(None, (up_stdout, up_stderr))
                        ).lower()
                        if "base network connection was interrupted" in combined_output:
                            print(
                                "   -> nmcli reported an interrupted base connection; retrying activation."
                            )
                            guest_command_executor(
                ...
                guest_command_executor(
                    f"ip -6 addr flush dev {device_name}", check_exit_code=False
                )
                guest_command_executor(
                    (
                ...
                if new_ip:
                    ...
                    if use_nmcli_connection:
                        verification_cmd = (
                            f"ip addr show {device_name} | grep -q '{new_ip}'"
                        )
                    else:
                        verification_cmd = (
                            legacy_verification_command
                            or f"ip addr show {device_name} | grep -q '{new_ip}'"
                        )
                ...
                if new_ip and prefix:
                    try:
                        ...
                        LOGGER.debug(
                            "Failed to derive fallback gateway from %s/%s",
                            new_ip,
                            prefix,
                            exc_info=True,
                        )
                ...
                for candidate in candidate_gateways:
                    if (
                        candidate
                        and candidate != new_ip
                        and candidate not in ping_targets
                    ):
                        ping_targets.append(candidate)
                LOGGER.debug(
                    "Connectivity targets for %s (%s): %s",
                    device_name,
                    new_ip,
                    ping_targets or "[none]",
                )
                try:
                    ensure_connection_activation(
                        guest_command_executor,
                    ...
                    decision = (
                        input(
                            "Continue despite the connectivity failure? (c=continue / a=abort): "
                        )
                        .strip()
                        .lower()
                    )
                    if decision == "c":
                        print(
                            "   -> Continuing despite failed connectivity checks per user direction."
                        )
                ...
                expected_ip_cidr = (
                    f"{new_ip}/{prefix}" if new_ip and prefix is not None else ""
                )
                if use_nmcli_connection:
        ...
        if not sdk_verification_succeeded:
            ...
            for (
                ...
                try:
                    ...
                    decision = (
                        input(
                            "Continue despite nmcli validation failure? (c=continue / a=abort): "
                        )
                        .strip()
                        .lower()
                    )
                    if decision == "c":
                        print(
                            "   -> Proceeding despite nmcli validation failure per user request."
                        )
                    else:
                        raise
        ...
        try:
            final_datastore = next(
                (
                    ds
                    for ds in final_view.view
                    if ds.name == TARGET_DATASTORE_NAME_FINAL
                ),
                None,
            )
        finally:
        ...
        if not final_datastore:
            raise FileNotFoundError(
                f"Final datastore '{TARGET_DATASTORE_NAME_FINAL}' was not found."
            )
        ...
        if cancellation_requested:
            cancellation_message = (
                str(error) or ""
            ).strip() or "Operation cancelled by the user."
        ...
        if migrated_vm_for_rollback:
            ...
            print(
                "\n"
                + "=" * 20
                + " Rollback Confirmation (Destination VM Removal) "
                + "=" * 20
            )
            print(
                "The process stopped, leaving a partially migrated VM on the destination vCenter."
            )
            ...
            if rollback_approval.lower() == "y":
                ...
                print(
                    "Rollback was cancelled; the VM remains on the destination vCenter."
                )
                dest_vm_retained = True
        if unregistered_from_source and not dest_vm_retained:
            print(
                "\n"
                + "=" * 20
                + " Rollback Confirmation (Datastore Cleanup) "
                + "=" * 20
            )
            print(
                "   Clone files remain on the source vCenter datastore and must be cleaned up."
            )
            print(
                f"   VM files may still exist on datastore '{TARGET_DATASTORE_NAME}'."
            )
            ...
            if rollback_approval_files.lower() == "y":
                ...
                try:
                    ...
                    if delete_state == vim.TaskInfo.State.success:
                        print(
                            "[OK] Rollback complete: removed files from the datastore."
                        )
                    else:
                        raise RuntimeError(
                ...
                print(
                    "File cleanup was cancelled by the user; files remain on the datastore."
                )
        elif new_vm_on_source:
            print(
                "\n"
                + "=" * 20
                + " Rollback Confirmation (Source VM Removal) "
                + "=" * 20
            )
            print(
                f"VM '{new_vm_on_source.name}' remains on the source vCenter from an interrupted run."
            )
            rollback_approval = input(
            ...
            if rollback_approval.lower() == "y":
                if new_vm_on_source.runtime.powerState == "poweredOn":
                    ...
                    if poweroff_state == vim.TaskInfo.State.success:
                        ...
                        print(
                            f"   [WARN] Power-off failed: {poweroff_task.info.error.msg}. Continuing with deletion."
                        )
        ...
        except (vmodl.MethodFault, IOError, RuntimeError) as disconnect_error:
            LOGGER.debug(
                "Error while disconnecting from destination vCenter: %s",
                disconnect_error,
                exc_info=True,
            )
        ...
        try:
            ...
            LOGGER.debug(
                "Error while disconnecting from source vCenter: %s",
                disconnect_error,
                exc_info=True,
            )
        if cancellation_requested:
